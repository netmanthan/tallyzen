...
frappe.ui.form.on("Dunning", {
	setup: function (frm) {
		...
		frm.set_query("company_address", tallyzen.queries.company_address_query);
	...
	company: function (frm) {
		if (frm.doc.company) {
			...
			if (frm.fields_dict.currency) {
				const company_currency = tallyzen.get_currency(frm.doc.company);
	...
	conversion_rate: function (frm) {
		if (frm.doc.currency === tallyzen.get_currency(frm.doc.company)) {
			frm.set_value("conversion_rate", 1.0);
		}

		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("conversion_rate", "read_only", tallyzen.stale_rate_allowed() ? 0 : 1);
	},
	customer_address: function (frm) {
		tallyzen.utils.get_address_display(frm, "customer_address");
	},
	company_address: function (frm) {
		tallyzen.utils.get_address_display(frm, "company_address");
	},
	...
	get_dunning_letter_text: function (frm) {
		if (frm.doc.dunning_type) {
			frappe.call({
				method:
					"tallyzen.accounts.doctype.dunning.dunning.get_dunning_letter_text",
				args: {
	...
	make_payment_entry: function (frm) {
		return frappe.call({
			method:
				"tallyzen.accounts.doctype.payment_entry.payment_entry.get_payment_entry",
			args: {
				dt: frm.doc.doctype,
				dn: frm.doc.name,
			},

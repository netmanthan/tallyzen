...
import tallyzen
from tallyzen.accounts.deferred_revenue import get_deferred_booking_accounts
from tallyzen.accounts.doctype.invoice_discounting.invoice_discounting import (
...
from tallyzen.controllers.accounts_controller import AccountsController
...
class JournalEntry(AccountsController):
	...
	def apply_tax_withholding(self):
		from tallyzen.accounts.report.general_ledger.general_ledger import get_account_type_map
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		from tallyzen.accounts.general_ledger import make_gl_entries
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": tallyzen.get_default_cost_center(self.company),
						},
					)
...
def get_default_bank_cash_account(company, account_type=None, mode_of_payment=None, account=None):
	from tallyzen.accounts.doctype.sales_invoice.sales_invoice import get_bank_cash_account
...
def get_outstanding(args):
	...
	company_currency = tallyzen.get_company_currency(args.get("company"))
	due_date = None
...
def get_account_balance_and_party_type(
	...
	company_currency = tallyzen.get_company_currency(company)
	account_details = frappe.get_cached_value(

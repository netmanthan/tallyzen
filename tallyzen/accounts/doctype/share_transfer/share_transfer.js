...
frappe.provide("tallyzen.share_transfer");
...
frappe.ui.form.on('Share Transfer', {
	refresh: function(frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.equity_or_liability_account && frm.doc.asset_account) {
			frm.add_custom_button(__('Create Journal Entry'), function () {
				tallyzen.share_transfer.make_jv(frm);
			});
		}
	...
	no_of_shares: (frm) => {
		if (frm.doc.rate != undefined || frm.doc.rate != null){
			tallyzen.share_transfer.update_amount(frm);
		}
	},
	rate: (frm) => {
...
tallyzen.share_transfer.update_amount = function(frm) {
	frm.doc.amount = frm.doc.no_of_shares * frm.doc.rate;
	frm.refresh_field("amount");
};

tallyzen.share_transfer.make_jv = function (frm) {
	...
	frappe.call({
		...
		method: "tallyzen.accounts.doctype.share_transfer.share_transfer.make_jv_entry",
		callback: function (r) {

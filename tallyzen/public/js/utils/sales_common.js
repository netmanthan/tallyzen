...
frappe.provide("tallyzen.selling");

tallyzen.sales_common = {
	setup_selling_controller:function() {
		tallyzen.selling.SellingController = class SellingController extends tallyzen.TransactionController {
			setup() {
				super.setup();
			...
			setup_queries() {
				...
				$.each([["customer", "customer"],
					...
					function(i, opts) {
						if(me.frm.fields_dict[opts[0]])
							me.frm.set_query(opts[0], tallyzen.queries[opts[1]]);
					});
				...
				me.frm.set_query('shipping_address_name', tallyzen.queries.address_query);
				me.frm.set_query('dispatch_address_name', tallyzen.queries.dispatch_address_query);
			...
			customer_address() {
				tallyzen.utils.get_address_display(this.frm, "customer_address");
				tallyzen.utils.set_taxes_from_address(this.frm, "customer_address", "customer_address", "shipping_address_name");
			}

			shipping_address_name() {
				tallyzen.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				tallyzen.utils.set_taxes_from_address(this.frm, "shipping_address_name", "customer_address", "shipping_address_name");
			}

			dispatch_address_name() {
				tallyzen.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			project() {
				...
				if(in_list(["Delivery Note", "Sales Invoice", "Sales Order"], this.frm.doc.doctype)) {
					if(this.frm.doc.project) {
						frappe.call({
							method:'tallyzen.projects.doctype.project.project.get_cost_center_name' ,

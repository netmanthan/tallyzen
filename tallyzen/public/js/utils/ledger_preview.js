...
tallyzen.accounts.ledger_preview = {
	show_accounting_ledger_preview(frm) {
		...
		if(!frm.is_new() && frm.doc.docstatus == 0) {
			frm.add_custom_button(__('Accounting Ledger'), function() {
				frappe.call({
					"type": "GET",
					"method": "tallyzen.controllers.stock_controller.show_accounting_ledger_preview",
					"args": {
	...
	show_stock_ledger_preview(frm) {
		...
		if(!frm.is_new() && frm.doc.docstatus == 0) {
			frm.add_custom_button(__('Stock Ledger'), function() {
				frappe.call({
					"type": "GET",
					"method": "tallyzen.controllers.stock_controller.show_stock_ledger_preview",
					"args": {
						"company": frm.doc.company,
						"doctype": frm.doc.doctype,
						"docname": frm.doc.name

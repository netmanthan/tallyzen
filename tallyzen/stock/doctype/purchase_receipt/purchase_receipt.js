...
tallyzen.accounts.taxes.setup_tax_validations("Purchase Receipt");
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_return === 1 && frm.doc.per_billed !== 100) {
			frm.add_custom_button(__('Debit Note'), function() {
				frappe.model.open_mapped_doc({
					method: "tallyzen.stock.doctype.purchase_receipt.purchase_receipt.make_purchase_invoice",
					frm: cur_frm,
				})
			}, __('Create'));
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_internal_supplier && !frm.doc.inter_company_reference) {
			frm.add_custom_button(__('Delivery Note'), function() {
				frappe.model.open_mapped_doc({
					method: 'tallyzen.stock.doctype.purchase_receipt.purchase_receipt.make_inter_company_delivery_note',
					frm: cur_frm,
				})
			}, __('Create'));
		}
		...
		if (frm.doc.docstatus == 0) {
			frm.add_custom_button(__('Purchase Invoice'), function () {
				...
				tallyzen.utils.map_current_doc({
					method: "tallyzen.accounts.doctype.purchase_invoice.purchase_invoice.make_purchase_receipt",
...
tallyzen.stock.PurchaseReceiptController = class PurchaseReceiptController extends tallyzen.buying.BuyingController {
	setup(doc) {
		...
		if (has_rejected_items && has_rejected_items.length > 0) {
			frappe.prompt([
				...
				if (values.return_for_rejected_warehouse) {
					frappe.call({
						method: "tallyzen.stock.doctype.purchase_receipt.purchase_receipt.make_purchase_return_against_rejected_warehouse",
						args: {
	...
	apply_putaway_rule() {
		if (this.frm.doc.apply_putaway_rule) tallyzen.apply_putaway_rule(this.frm);
	}

};

// for backward compatibility: combine new and previous states
extend_cscript(cur_frm.cscript, new tallyzen.stock.PurchaseReceiptController({frm: cur_frm}));

cur_frm.cscript.update_status = function(status) {
	frappe.ui.form.is_saving = true;
	frappe.call({
		method:"tallyzen.stock.doctype.purchase_receipt.purchase_receipt.update_purchase_receipt_status",
...
cur_frm.cscript._make_purchase_return = function() {
	frappe.model.open_mapped_doc({
		method: "tallyzen.stock.doctype.purchase_receipt.purchase_receipt.make_purchase_return",
		frm: cur_frm
	});
}
...
cur_frm.cscript['Make Stock Entry'] = function() {
	frappe.model.open_mapped_doc({
		method: "tallyzen.stock.doctype.purchase_receipt.purchase_receipt.make_stock_entry",
		frm: cur_frm,
	})
}

...
frappe.ui.form.on('Pick List', {
	setup: (frm) => {
		...
		frm.set_query('work_order', () => {
			return {
				query: 'tallyzen.stock.doctype.pick_list.pick_list.get_pending_work_orders',
				filters: {
					'company': frm.doc.company
				}
			};
	...
	refresh: (frm) => {
		...
		if (frm.doc.docstatus === 1) {
			frappe.xcall('tallyzen.stock.doctype.pick_list.pick_list.target_document_exists', {
		...
		frappe.db.get_value('Work Order',
			...
			frappe.prompt({
				...
				tallyzen.utils.map_current_doc({
					method: 'tallyzen.manufacturing.doctype.work_order.work_order.create_pick_list',
					target: frm,
	...
	create_delivery_note: (frm) => {
		frappe.model.open_mapped_doc({
			method: 'tallyzen.stock.doctype.pick_list.pick_list.create_delivery_note',
			frm: frm
		});

	},
	create_stock_entry: (frm) => {
		frappe.xcall('tallyzen.stock.doctype.pick_list.pick_list.create_stock_entry', {
			'pick_list': frm.doc,
		}).then(stock_entry => {
	...
	scan_barcode: (frm) => {
		...
		const barcode_scanner = new tallyzen.utils.BarcodeScanner(opts);
...
function get_item_details(item_code, uom=null) {
	if (item_code) {
		return frappe.xcall('tallyzen.stock.doctype.pick_list.pick_list.get_item_details', {
			item_code,
			uom
		});
	}

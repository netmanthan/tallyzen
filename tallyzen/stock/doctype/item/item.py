...
from tallyzen.setup.doctype.item_group.item_group import invalidate_cache_for
from tallyzen.stock.doctype.item_default.item_default import ItemDefault
...
class Item(Document):
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": tallyzen.get_default_currency(),
			...
			for item_barcode in self.barcodes:
				...
				if item_barcode.barcode:
					...
					if item_barcode.barcode_type:
						barcode_type = convert_tallyzen_to_barcodenumber(
	...
	def recalculate_bin_qty(self, new_name):
		from tallyzen.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"tallyzen.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
...
def convert_tallyzen_to_barcodenumber(tallyzen_number, barcode):
	if tallyzen_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
		}
	...
	if tallyzen_number in convert:
		return convert[tallyzen_number]
...
def invalidate_item_variants_cache_for_website(doc):
	...
	from tallyzen.e_commerce.variant_selector.item_variants_cache import ItemVariantsCacheManager
...
def get_asset_naming_series():
	from tallyzen.assets.doctype.asset.asset import get_asset_naming_series

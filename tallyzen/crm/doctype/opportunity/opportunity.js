...
frappe.provide("tallyzen.crm");
tallyzen.pre_sales.set_as_lost("Opportunity");
...
frappe.ui.form.on("Opportunity", {
	...
	party_name: function(frm) {
		...
		if (frm.doc.opportunity_from == "Customer") {
			tallyzen.utils.get_party_details(frm);
		} else if (frm.doc.opportunity_from == "Lead") {
			tallyzen.utils.map_current_doc({
				method: "tallyzen.crm.doctype.lead.lead.make_opportunity",
	...
	customer_address: function(frm, cdt, cdn) {
		tallyzen.utils.get_address_display(frm, 'customer_address', 'address_display', false);
	},

	contact_person: tallyzen.utils.get_contact_details,
	...
	currency: function(frm) {
		let company_currency = tallyzen.get_currency(frm.doc.company);
	...
	make_supplier_quotation: function(frm) {
		frappe.model.open_mapped_doc({
			method: "tallyzen.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm
		})
	},
	...
	make_request_for_quotation: function(frm) {
		frappe.model.open_mapped_doc({
			method: "tallyzen.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm
		})
	},
...
tallyzen.crm.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	onload() {
	...
	setup_queries() {
		...
		me.frm.set_query('customer_address', tallyzen.queries.address_query);
		...
		if (me.frm.doc.opportunity_from == "Lead") {
			me.frm.set_query('party_name', tallyzen.queries['lead']);
		}
		else if (me.frm.doc.opportunity_from == "Customer") {
			me.frm.set_query('party_name', tallyzen.queries['customer']);
	...
	create_quotation() {
		frappe.model.open_mapped_doc({
			method: "tallyzen.crm.doctype.opportunity.opportunity.make_quotation",
			frm: cur_frm
		})
	}
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "tallyzen.crm.doctype.opportunity.opportunity.make_customer",
			frm: cur_frm
		})
	}

	show_notes() {
		const crm_notes = new tallyzen.utils.CRMNotes({
			frm: this.frm,
	...
	show_activities() {
		const crm_activities = new tallyzen.utils.CRMActivities({
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new tallyzen.crm.Opportunity({frm: cur_frm}));

cur_frm.cscript.item_code = function(doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return frappe.call({
			method: "tallyzen.crm.doctype.opportunity.opportunity.get_item_details",

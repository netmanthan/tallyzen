...
tallyzen.accounts.taxes.setup_tax_validations("Sales Order");
...
frappe.ui.form.on("Sales Order", {
	...
	onload: function(frm) {
		...
		frm.set_query('project', function(doc, cdt, cdn) {
			return {
				query: "tallyzen.controllers.queries.get_project_name",
				filters: {
		...
		frm.set_query('warehouse', 'items', function(doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "tallyzen.controllers.queries.warehouse_query";
	...
	cancel_stock_reservation_entries(frm) {
		...
		frappe.call({
			method: 'tallyzen.stock.doctype.stock_reservation_entry.stock_reservation_entry.get_stock_reservation_entries_for_voucher',
			args: {
...
frappe.ui.form.on("Sales Order Item", {
	...
	delivery_date: function(frm, cdt, cdn) {
		if(!frm.doc.delivery_date) {
			tallyzen.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	}
});

tallyzen.selling.SalesOrderController = class SalesOrderController extends tallyzen.selling.SellingController {
	onload(doc, dt, dn) {
	...
	refresh(doc, dt, dn) {
		...
		if (this.frm.doc.docstatus===0) {
			this.frm.add_custom_button(__('Quotation'),
				function() {
					let d = tallyzen.utils.map_current_doc({
						method: "tallyzen.selling.doctype.quotation.quotation.make_sales_order",
	...
	create_pick_list() {
		frappe.model.open_mapped_doc({
			method: "tallyzen.selling.doctype.sales_order.sales_order.create_pick_list",
			frm: this.frm
		})
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "tallyzen.selling.doctype.sales_order.sales_order.get_work_order_items",
			args: {
	...
	make_material_request() {
		frappe.model.open_mapped_doc({
			method: "tallyzen.selling.doctype.sales_order.sales_order.make_material_request",
			frm: this.frm
		})
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new frappe.ui.Dialog({
			...
			primary_action: function() {
				...
				me.frm.call({
					method: 'tallyzen.selling.doctype.sales_order.sales_order.make_raw_material_request',
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
	...
	make_delivery_note(delivery_dates, for_reserved_stock=false) {
		frappe.model.open_mapped_doc({
			method: "tallyzen.selling.doctype.sales_order.sales_order.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates,
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "tallyzen.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
			frm: this.frm
		})
	}
	...
	make_project() {
		frappe.model.open_mapped_doc({
			method: "tallyzen.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm
		})
	}
	...
	make_inter_company_order() {
		frappe.model.open_mapped_doc({
			method: "tallyzen.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm
		});
	}
	...
	make_maintenance_visit() {
		frappe.model.open_mapped_doc({
			method: "tallyzen.selling.doctype.sales_order.sales_order.make_maintenance_visit",
			frm: this.frm
		})
	}
	...
	make_purchase_order(){
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action (args) {
				...
				return frappe.call({
					method: "tallyzen.selling.doctype.sales_order.sales_order." + method,
	...
	update_status(label, status){
		...
		frappe.call({
			method: "tallyzen.selling.doctype.sales_order.sales_order.update_status",
...
extend_cscript(cur_frm.cscript, new tallyzen.selling.SalesOrderController({frm: cur_frm}));
